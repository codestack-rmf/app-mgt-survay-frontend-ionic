...
import { IonPage, IonContent, IonCard, IonCardContent, IonInput, IonItem, IonLabel, IonButton, IonToggle, IonRange, IonRadio, IonRadioGroup, IonText } from '@ionic/react';
...
const questions = [
  { id: 'theList',
    ...
    options: [
      'Random cards I’ve collected.',
      'Some good cards, but missing obvious upgrades.',
      'Optimized and synergistic, favoring theme over meta.',
      'Strongest cards, no budget, no restrictions.',
    ]
  },
  { id: 'manabase',
    question: 'How would you describe your deck’s manabase?',
    options: [
      '28 lands is fine, right?',
      'Lands that enter tapped, I need better fixing.',
      'Optimized manabase, fetchable lands.',
      'Perfect manabase, top-tier fixing.',
    ]
  },
  { id: 'strategy', 
    question: 'How clear and reinforced is your deck’s strategy?',
    options: [
      'No defined plan, just chaos.',
      'Strategy is unclear or not reinforced.',
      'Clear strategy, consistent execution.',
      'Winning is the only strategy.',
    ]
  },
  {
    id: 'winCondition',
  ...
  {
    id: 'commanderStaples',
    question:'What format card archetype best describes your deck?',
    options: [
  ...
  {
    id: 'winArchetype',
    question: 'What best describes your win strategy?',
    options: [
          ...
          <IonCardContent>
            {step === 0 ? (
              <>
                ...
                <IonItem>
                  <IonLabel>Import Decktlist</IonLabel>
                  <IonToggle checked={deckImport} onIonChange={(e) => setDeckImport(e.detail.checked)} />
                </IonItem>

                {deckImport && (
                  <IonItem className="rounded-input">
                    <IonLabel position="floating">Deck URL</IonLabel>
                    <IonInput value={deckUrl} onIonChange={(e) => setDeckUrl(e.detail.value!)} />
                  </IonItem>
                )}

                <IonItem className="rounded-input">
                  <IonLabel position="floating">Deck Name</IonLabel>
                  <IonInput value={deckName} onIonChange={(e) => setDeckName(e.detail.value!)} />
                </IonItem>

                <IonItem className="rounded-input">
                  <IonLabel position="floating">Commander</IonLabel>
                  <IonInput value={selectedCommander} onIonChange={(e) => setSelectedCommander(e.detail.value!)} />
                </IonItem>
                ...
                {usePartnerCommander && (
                  <IonItem className="rounded-input">
                    <IonLabel position="floating">Partner Commander</IonLabel>
                    <IonInput value={partnerCommander} onIonChange={(e) => setPartnerCommander(e.detail.value!)} />
                  </IonItem>
                )}

                <IonItem>
                  <IonLabel>Player Experience</IonLabel>
                  <IonRange ticks={true} snaps={true} min={1} max={4} step={1} value={playerExperience} onIonChange={(e) => setPlayerExperience(Number(e.detail.value))} />
                </IonItem>

                {errorMessage && <IonText color="danger">{errorMessage}</IonText>}

                <IonButton expand="full" className="rounded-button" onClick={handleNext}>
                  Next
                </IonButton>
              </>
              ...
              <>
                ...
                <div className="survey-buttons">
                  <IonButton className="rounded-button" onClick={handleBack}>
                    Back
                  </IonButton>
                  <IonButton className="rounded-button" onClick={handleNext}>
